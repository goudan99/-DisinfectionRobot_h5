...
/**
 * @description 清空标签导航列表
 */
export const clearTagNav = () => {
...
export const formatRouters = (array, access) => {
  ...
  const error_404 = {
    ...
    component: () => import('@/view/system/error/404.vue')
  }
  const error_401 = {
    path: '*',
    name: 'error_401',
    ...
    component: () => import('@/view/system/error/401.vue')
  }
  const error_500 = {
    path: '*',
    name: 'error_500',
    ...
    component: () => import('@/view/system/error/500.vue')
  }
  // 放到最后
  routers.push(error_404)
  routers.push(error_401)
  routers.push(error_500)
  return routers
}
...
export const filterRouter = (array, access, routers) => {
  let list = array.map(item => {
    ...
    if (parseInt(item.parent_id) === 0 || parseInt(item.id) === 0) {
      // 根节点
      router.path = '/'
      ...
      if (item.target === '_blank') {
        ...
        if (item.prefix === '/') {
          // 内部组件
          router.component = (resolve) => {
            require([`@/view/${item.path}.vue`], resolve)
          }
        } else {
          // 传递iframe路径参数
...
export const listToTree = (array, startPid, currentDept, opt) => {
  ...
  if (array && array.length > 0) {
    child = array.map(item => {
      ...
      if (typeof item[opt.parentKey] !== 'undefined' && parseInt(item[opt.parentKey]) === startPid) {
        // 满足条件则递归
