  ...
  <div>
    <Card shadow>
      ...
      <div class="search-con search-con-top">
        <Input placeholder="请输入任务名,或设备名..." style="width: 300px" v-model="search.key"></Input>
...
import { getJobLogs } from '@/api/app'
...
export default {
  ...
  methods: {
    handleSearch () {
      this.loading = true
      this.seloading = true
      getJobLogs(Object.assign({}, this.pageInfo, this.search)).then(res => {
        if (res.code === 0) {
    ...
    handleExcel () {
      if (this.data.length) {
        ...
        const params = {
          title: ['任务名', '执行设备', '执行任务', '当前状态', '时间'],
          key: ['job_name', 'machine_name', 'action', 'status', 'created_at'],
          data: this.data,
          autoWidth: true,
          filename: '任务日志'
        }
